...
namespace datamodel.graphviz {
    public static class GraphvizRunner {
        ...
        public static void Run(string inputPath, string outputPath, RenderingStyle style) {
            ...
            string commandLine = string.Format("-Tsvg -o{0} -Gimagepath=.. {1}", outputPath, inputPath);
            ...
            if (!File.Exists(outputPath)) {
                ...
                throw new Exception(string.Format("Error processing Graphviz {0} file '{1}'. Exit Code: {2}", exec, inputPath, process.ExitCode));
            }
        }
    }
}
